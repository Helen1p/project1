...
def immeanshift(im,level, i) :

    # levels = [0.15, 0.08, 0, -0.08, -0.15]
    levels = [(0.15, -0.15), (0.22, -0.22), (0.3, -0.3), (0.37, -0.37), (0.46, -0.46)]
    amount = levels[level][i]
...
def imresizedist_bicubic(im,level) :

    # levels = [2,3,4,8,16]
    levels = [1.2,1.5,1.8,2.1,2.4]
    amount = levels[level]
...
def imcolorblock(im, level):
    ...
    for i in range(0, param):
        ...
        im[y:y+patch_size[0], x:x+patch_size[1],:] = color
...
def imnoneccentricity(im, level):
    ...
    for i in range(0, param):
        ...
        print('rand_h_start',rand_h_start)
        print('rand_w_start',rand_w_start)
